...
public class GridBehavior : MonoBehaviour
{
        ...
        for (int i = 0; i < columns; i++)
        {
            for (int j = 0; j < rows; j++)
            {
                GameObject obj = Instantiate(gridPrefab,
                    new Vector3(leftBottomLocation.x + scale * i, leftBottomLocation.y,
                        leftBottomLocation.z + scale * j), Quaternion.identity);
    ...
    void InitialSetup()
    {
        ...
        gridArray[startX, startY]
            .GetComponent<GridStat>().visited = 0;
    }
